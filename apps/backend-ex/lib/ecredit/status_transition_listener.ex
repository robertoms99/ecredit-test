defmodule Ecredit.StatusTransitionListener do
  ...
  def init(_opts) do
    ...
    Logger.info("StatusTransitionListener iniciado, escuchando canal status_transition")
    {:ok, %{pg_notifications: pid}}
  end

  @impl true
  def handle_info({:notification, _pid, _ref, "status_transition", payload}, state) do
    Logger.debug("Notificación status_transition recibida: #{payload}")
    ...
    case Jason.decode(payload) do
      ...
      {:error, reason} ->
        Logger.error("Error al decodificar el payload de notificación: #{inspect(reason)}")
    end
  ...
  defp handle_transition_notification(%{
    ...
    StatusTransitionWorker.enqueue(%{
      credit_request_id: credit_request_id,
      updated_at: updated_at,
      request_status_id: to_status_id,
    ...
    Logger.info("Actualización de estado difundida para solicitud #{credit_request_id}")
  end

  defp handle_transition_notification(data) do
    Logger.warning("Formato de notificación inesperado: #{inspect(data)}")
  end
end
